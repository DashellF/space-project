...
@export var orbit_radius: float = 141600 #radius
@export var orbit_speed: float = -0.000381 # radians per hour

var angle := deg_to_rad(108.8786)
var mesh: Node3D
...
func _on_hours_updated(new_value: float):
	angle = deg_to_rad(108.8786) + orbit_speed * new_value
	
func _on_timespeed_updated(new_value: float):
	orbit_speed = -0.000381 * global.timeSpeed
func _get_position_in_time(hours):
	var newAngle = deg_to_rad(108.8786) + orbit_speed * hours
