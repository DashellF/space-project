...
func update_global_time():
	...
	global.month = int(raw_digits[0] + raw_digits[1])
	global.day = int(raw_digits[2] + raw_digits[3])
	global.year = int(raw_digits[4] + raw_digits[5] + raw_digits[6] + raw_digits[7])
	global.hour = int(raw_digits[8] + raw_digits[9])
	global.minute = int(raw_digits[10] + raw_digits[11])

	global.HOURSFROMBASETIME = global.get_hours_from_format()
	global.updated_time = text  # optional, stores formatted string
